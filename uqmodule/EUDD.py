...
class ResNetVAEWithFeature(nn.Module):
    ...
    def forward(self, x):
        ...
        class_output2 = self.fc_class2(mu)

        return class_output1, class_output2

class pretrained(LightningModule):
    def __init__(self, class_num=10, model_name = 'resnet18', p = 30):
        super(pretrained, self).__init__()

        # 모델 생성
        self.model = ResNetVAEWithFeature( num_classes = class_num, model_name = model_name)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss1 = self.criterion2(class_output1,labels)
        loss2 = self.criterion1(class_output2,labels)
        loss = loss1 + 0.01 * loss2
        #loss = loss2 + 0.001 * loss3
...
def inference(model, test_loader, threshold, Temperautue):
    ...
    with torch.no_grad():
        ...
        # 모델 예측
            ...
            distances.extend(distance1)
            entropys.extend(calculate_entropy(prob))
