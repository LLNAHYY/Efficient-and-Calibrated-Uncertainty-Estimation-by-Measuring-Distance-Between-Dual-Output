...
from uqmodule.duq import DUQ, inference, ood_inference
import argparse
...
def main():
    ...
    parser.add_argument('--l_gradient_penalty',          type=float,   default=0.01)
    parser.add_argument('--seeds', type=lambda s: [int(item) for item in s.split(',')], default=[0,93,777])
    # args 에 위의 내용 저장
    ...
    #epoch = 10
    qu_method = 'duq'
    #model_name = 'mnist' #resnet18 , resnet34
    ...
    if train_true: 
        for seed in args.seeds:#arg
            ...
            model = DUQ(args.num_classes, args.model_name, args.l_gradient_penalty)
